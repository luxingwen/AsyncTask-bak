...
func (r *RDB) runScript(op Op, script *redis.Script, keys []string, args ...interface{}) error {
	if err := script.Run(context.Background(), r.client, keys, args...).Err(); err != nil {
		return Err(op, errors.Internal, fmt.Sprintf("redis eval error: %v", err))
	}
	return nil
}
...
func (r *RDB) Enqueue(msg *TaskMessage) error {
	...
	if err := r.client.SAdd(context.Background(), AllQueues, msg.Queue).Err(); err != nil {
		return errors.E(op, errors.Unknown, &errors.RedisCommandError{Command: "sadd", Err: err})
	}
	keys := []string{
		TaskKey(msg.Queue, msg.ID.String()),
...
func (r *RDB) Dequeue(qnames ...string) (msg *TaskMessage, deadline time.Time, err error) {
	...
	for _, qname := range qnames {
		...
		if msg, err = DecodeMessage([]byte(encoded)); err != nil {
...
func (r *RDB) Done(msg *TaskMessage) error {
	...
	keys := []string{
		ActiveKey(msg.Queue),
		DeadlinesKey(msg.Queue),
		TaskKey(msg.Queue, msg.ID.String()),
		ProcessedKey(msg.Queue, now),
	}
	argv := []interface{}{
		msg.ID.String(),
		expireAt.Unix(),
...
// ARGV[1] -> task ID
// ARGV[2] -> updated TaskMessage value
...
func (r *RDB) Retry(msg *TaskMessage, processAt time.Time, errMsg string, isFailure bool) error {
...
func (r *RDB) Archive(msg *TaskMessage, errMsg string) error {
...
func (r *RDB) forwardAll(qname string) (err error) {
	sources := []string{ScheduledKey(qname), RetryKey(qname)}
	dst := PendingKey(qname)
	taskKeyPrefix := TaskKeyPrefix(qname)
...
func (r *RDB) ListDeadlineExceeded(deadline time.Time, qnames ...string) ([]*TaskMessage, error) {
	...
	for _, qname := range qnames {
		res, err := listDeadlineExceededCmd.Run(context.Background(), r.client,
			[]string{DeadlinesKey(qname)},
			deadline.Unix(), TaskKeyPrefix(qname)).Result()
		if err != nil {
...
func (r *RDB) WriteServerState(info *ServerInfo, workers []*WorkerInfo, ttl time.Duration) error {
	var op errors.Op = "rdb.WriteServerState"
	bytes, err := EncodeServerInfo(info)
	if err != nil {
	...
	for _, w := range workers {
		bytes, err := EncodeWorkerInfo(w)
		if err != nil {
			continue // skip bad data
		}
		args = append(args, w.ID, bytes)
	}
	skey := ServerInfoKey(info.Host, info.PID, info.ServerID)
	wkey := WorkersKey(info.Host, info.PID, info.ServerID)
	if err := r.client.ZAdd(context.Background(), AllServers, &redis.Z{Score: float64(exp.Unix()), Member: skey}).Err(); err != nil {
		return errors.E(op, errors.Unknown, &errors.RedisCommandError{Command: "sadd", Err: err})
	}
	if err := r.client.ZAdd(context.Background(), AllWorkers, &redis.Z{Score: float64(exp.Unix()), Member: wkey}).Err(); err != nil {
...
func (r *RDB) ClearServerState(host string, pid int, serverID string) error {
	...
	skey := ServerInfoKey(host, pid, serverID)
	wkey := WorkersKey(host, pid, serverID)
	if err := r.client.ZRem(context.Background(), AllServers, skey).Err(); err != nil {
		return errors.E(op, errors.Internal, &errors.RedisCommandError{Command: "zrem", Err: err})
	}
	if err := r.client.ZRem(context.Background(), AllWorkers, wkey).Err(); err != nil {
		return errors.E(op, errors.Internal, &errors.RedisCommandError{Command: "zrem", Err: err})
	}
...
func (r *RDB) WriteSchedulerEntries(schedulerID string, entries []*SchedulerEntry, ttl time.Duration) error {
	...
	exp := time.Now().Add(ttl).UTC()
	key := SchedulerEntriesKey(schedulerID)
	err := r.client.ZAdd(context.Background(), AllSchedulers, &redis.Z{Score: float64(exp.Unix()), Member: key}).Err()
	if err != nil {
		return errors.E(op, errors.Unknown, &errors.RedisCommandError{Command: "zadd", Err: err})
	}
...
func (r *RDB) ClearSchedulerEntries(scheduelrID string) error {
	...
	key := SchedulerEntriesKey(scheduelrID)
	if err := r.client.ZRem(context.Background(), AllSchedulers, key).Err(); err != nil {
...
func (r *RDB) CancelationPubSub() (*redis.PubSub, error) {
	...
	pubsub := r.client.Subscribe(context.Background(), CancelChannel)
...
func (r *RDB) PublishCancelation(id string) error {
	...
	if err := r.client.Publish(context.Background(), CancelChannel, id).Err(); err != nil {
...
func (r *RDB) RecordSchedulerEnqueueEvent(entryID string, event *SchedulerEnqueueEvent) error {
...
func (r *RDB) ClearSchedulerHistory(entryID string) error {
	...
	key := SchedulerHistoryKey(entryID)
	if err := r.client.Del(context.Background(), key).Err(); err != nil {
		return errors.E(op, errors.Unknown, &errors.RedisCommandError{Command: "del", Err: err})
	}
	return nil
